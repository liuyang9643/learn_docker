...
![image-20210417145809484](\img\DockerDockerfile\image-20210417145809484.png)
...
打开浏览器，地址栏输入：IP，可以看到首页内容已经换成了我们在制作 Dockerfile 时使用 echo 输出的信息。
...
指定工作目录。用 WORKDIR 指定的工作目录，会在构建镜像的每一层中都存在。注意：WORKDIR 指定的工作目录，必须是提前创建好的。
